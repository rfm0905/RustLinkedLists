...
impl<T> List<T> {
    ...
    pub fn push_front(&mut self, elem: T) {
        ...
        match self.head.take() {
            Some(oldhead) => {
                oldhead.borrow_mut().prev = Some(newhead.clone()); // use borrow_mut to use interior mutability
                newhead.borrow_mut().next = Some(oldhead);
                self.head = Some(newhead);
            }
            None => {
                self.tail = Some(newhead.clone());
                self.head = Some(newhead);
            }
        }
    }
    ...
    pub fn pop_front(&mut self) -> Option<T> {
        self.head.take().map(|oldhead| {
            ...
            Rc::try_unwrap(oldhead).ok().unwrap().into_inner().elem // destroys interior mutability
        })
    }
